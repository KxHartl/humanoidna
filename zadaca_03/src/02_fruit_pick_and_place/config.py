...
OUTPUT_DIR = DATA_DIR / "raw" / "outputs"
...
class PathsConfig:
    ...
    # Use the trained fruit segmentation model locally
    segmentation_model: Path = MODELS_DIR / "yolo26n-seg_trained.pt"

    captures_dir: Path = DATA_DIR / "raw" / "video"  # or somewhere appropriate
    scene_dir: Path = DATA_DIR / "raw" / "scene"
...
class PickPlaceConfig:
    ...
    # TU !!!
    place_pose: List[float] = field(default_factory=lambda: [-0.42204, 0.29415, 0.050, 1.287, 2.822, -0.064])

    approach_offset_z: float = 0.100
    grasp_offset_z: float = -0.020
